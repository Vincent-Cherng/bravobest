...
@Slf4j
public class PaginationInterceptor1 implements Interceptor {
    ...
    public Object intercept(Invocation invocation) throws Throwable {
        ...
        if (isSelect(boundSql.getSql())) {
            ...
            if (params instanceof Map) { // 请求为多个参数，参数采用Map封装
                ...
                return simpleParamHandler(invocation, boundSql, (Pager) params);
            }
        }
    ...
    private Object complexParamsHandler(Invocation invocation, BoundSql boundSql, Map<?, ?> params) throws Throwable {
        ...
        if (containsPage(params)) {
            return pageHandlerExecutor(invocation, boundSql, (Pager) params.get("page"));
        } else {
            return invocation.proceed();
        }
    }
    ...
    private Object simpleParamHandler(Invocation invocation, BoundSql boundSql, Pager pager) throws Throwable {
        return pageHandlerExecutor(invocation, boundSql, pager);
    }

    private Object pageHandlerExecutor(Invocation invocation, BoundSql boundSql, Pager pager) throws Throwable {
        // 获得数据库连接
        ...
        // 生成分页sql
        pager.setTotalRecord(maxSize);
        String wrapperSql = getPageSql(boundSql.getSql(), pager);
